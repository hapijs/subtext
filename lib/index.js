    ...
    return new Promise((resolve, reject) => {
        ...
        const onPart = async (part) => {
            ...
            if (output === 'file') {                                                                // Output: 'file'
                const writePromise = internals.writeFile(req, options, part);

                // Register a fake error handler so we don't trigger node's unhandled rejections handler,
                // the error will bubble in finalize's Promise.all at a later time.
                writePromise.catch(Hoek.ignore);

                pendingFiles.push(writePromise);
            }
            else {                                                                                  // Output: 'data'
