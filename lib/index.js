...
exports.registerMimeTypeHandler = function (mimeType, handler) {

  Hoek.assert(mimeType, 'Missing mime type');
  Hoek.assert(typeof handler === 'function', 'Missing mime type handler function');

  internals.mimeTypeHandlers[mimeType] = handler;
};
...
internals.object = function (payload, mime, next) {
    ...
    for (var mimeType in internals.mimeTypeHandlers) {
        if (mime === mimeType) {
            var handler = internals.mimeTypeHandlers[mimeType];
            return internals.customParse(payload, next, handler);
        }
    }
...
internals.customParse = function (payload, next, handler) {

  try {
      return handler(payload, next);
  }
  catch (err) {
      return next(Boom.badRequest('Invalid request payload format', err));
  }
};
