...
internals.object = function (payload, mime, next) {
    ...
    if (/^application\/(?:.+\+)?xml$/.test(mime)) {
      return internals.xmlParse(payload, next);                        // Isolate try...catch for V8 optimization
    }
...
internals.xmlParse = function (payload, next) {

    try {
        return Xml2Js.parseString(payload.toString('utf8'), { explicitArray: false }, next);
    }
    catch (err) {
        return next(Boom.badRequest('Invalid request payload XML format', err));
    }
};
