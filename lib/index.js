...
internals.Parser.prototype.parse = function (contentType) {
    ...
    var qsOptions = this.settings.qsOptions || {};
    // Content-encoding
    ...
    return Wreck.read(source, { timeout: this.settings.timeout, maxBytes: this.settings.maxBytes }, function (err, payload) {
        ...
        internals.object(payload, self.result.contentType.mime, qsOptions, function (err, result) {
...
internals.object = function (payload, mime, qsOptions, next) {
    ...
    if (mime === 'application/x-www-form-urlencoded') {
        return next(null, Qs.parse(payload.toString('utf8'), qsOptions));
    }
